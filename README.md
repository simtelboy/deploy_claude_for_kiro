...
如需手动启动 AIClient-2-API，请使用以下命令：

```powershell
cd {安装目录}\AIClient-2-API
node src/services/api-server.js
```

> **注意**：新版本的启动入口从 `src/api-server.js` 变更为 `src/services/api-server.js`

## 配置文件位置

- **Claude 配置**：`%USERPROFILE%\.claude\settings.json`
- **Router 配置**：`%USERPROFILE%\.claude-code-router\config.json`
- **AIClient 配置**：`{安装目录}\AIClient-2-API\configs\config.json` (新版本路径)
- **AIClient 号池配置**：`{安装目录}\AIClient-2-API\configs\provider_pools.json`
- **Kiro Token**：`%USERPROFILE%\.aws\sso\cache\kiro-auth-token.json`

> **注意**：AIClient-2-API 新版本的配置文件路径从根目录 `config.json` 变更为 `configs/config.json`
