...
if ($installChoice -eq "y") {
    ...
    # AIClient config.json
    $aiclientConfigPath = "$hotyiDevPath\AIClient-2-API\config.json"
    ...
    Write-Host "- AIClient config.json 已生成" -ForegroundColor Green
...
taskkill /F /IM node.exe /FI "WINDOWTITLE eq Router*" /T 2>`$null
taskkill /F /IM node.exe /FI "WINDOWTITLE eq MCP*" /T 2>`$null
Start-Process -FilePath "node" -ArgumentList "src/api-server.js --log-prompts file --request-max-retries 5" -WorkingDirectory "$hotyiDevPath\AIClient-2-API"
"@
